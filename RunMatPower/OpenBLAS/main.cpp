...
//    TM.NumThreads= 4;
    ...
    string MatMultOption="OpenBLAS";
    
    openblas_set_num_threads(TM.NumThreads); // set number of threads in OpenBLAS
    ...
    I=(double*) malloc(((TM.PopSize)+1)*((TM.PopSize)+1)*sizeof(double));
    
    m=(double*) malloc(((TM.PopSize)+1)*((TM.PopSize)+1)*sizeof(double));
    ...
    //std::string filename = std::string("./")+ std::string("MatPow_Pow")+PowString+std::string("_PopSize")+PopSizeString+std::string("_Sigma")+SigmaString+std::string("_")+MatMultOption+std::string(".dat"); // filename
